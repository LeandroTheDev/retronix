...
import 'playstation1_game_open.dart';

class Playstation1GameDetailsPage extends StatefulWidget {
  const Playstation1GameDetailsPage({super.key, required this.gameName});

  final String gameName;

  @override
  State<Playstation1GameDetailsPage> createState() => _Playstation1GameDetailsPageState();
}

class _Playstation1GameDetailsPageState extends State<Playstation1GameDetailsPage> {
  static const _console = 'Playstation 1';
  ...
  void initState() {
    ...
    DebugLogger.log('[Playstation1GameDetailsPage] initState — subscribed to gamepad stream');
    _load();
  }
  ...
  Future<void> _load() async {
    ...
    if (await defFile.exists()) {
      try {
        ...
        DebugLogger.log('[Playstation1GameDetailsPage] failed to parse achievements: $e');
      }
    }
    ...
    if (await progressFile.exists()) {
      try {
        ...
        DebugLogger.log('[Playstation1GameDetailsPage] failed to parse progress: $e');
      }
    }
    ...
    if (await playtimeFile.exists()) {
      try {
        ...
        DebugLogger.log('[Playstation1GameDetailsPage] failed to parse playtime: $e');
      }
    }
  ...
  void _handleAction(GamepadAction action) {
    DebugLogger.log('[Playstation1GameDetailsPage] _handleAction: $action isCurrent=${ModalRoute.of(context)?.isCurrent}');
  ...
  Future<void> _playGame() async {
    final error = await Navigator.push<String>(
      context,
      MaterialPageRoute(builder: (_) => Playstation1GameOpen(gameName: widget.gameName)),
    );
    if (!mounted) return;
    if (error != null) {
  ...
  Future<void> _confirmResetAchievements(AppLocalizations l) async {
    ...
    DebugLogger.log('[Playstation1GameDetailsPage] achievements reset for ${widget.gameName}');
