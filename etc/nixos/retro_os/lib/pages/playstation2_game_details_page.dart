...
import 'playstation2_game_open.dart';

class Playstation2GameDetailsPage extends StatefulWidget {
  const Playstation2GameDetailsPage({super.key, required this.gameName});

  final String gameName;

  @override
  State<Playstation2GameDetailsPage> createState() => _Playstation2GameDetailsPageState();
}

class _Playstation2GameDetailsPageState extends State<Playstation2GameDetailsPage> {
  static const _console = 'Playstation 2';
  ...
  void initState() {
    ...
    DebugLogger.log('[Playstation2GameDetailsPage] initState — subscribed to gamepad stream');
    _load();
  }
  ...
  Future<void> _load() async {
    ...
    if (await defFile.exists()) {
      try {
        ...
        DebugLogger.log('[Playstation2GameDetailsPage] failed to parse achievements: $e');
      }
    }
    ...
    if (await progressFile.exists()) {
      try {
        ...
        DebugLogger.log('[Playstation2GameDetailsPage] failed to parse progress: $e');
      }
    }
    ...
    if (await playtimeFile.exists()) {
      try {
        ...
        DebugLogger.log('[Playstation2GameDetailsPage] failed to parse playtime: $e');
      }
    }
  ...
  void _handleAction(GamepadAction action) {
    DebugLogger.log('[Playstation2GameDetailsPage] _handleAction: $action isCurrent=${ModalRoute.of(context)?.isCurrent}');
  ...
  Future<void> _playGame() async {
    final error = await Navigator.push<String>(
      context,
      MaterialPageRoute(builder: (_) => Playstation2GameOpen(gameName: widget.gameName)),
    );
    if (!mounted) return;
    if (error != null) {
  ...
  Future<void> _confirmResetAchievements(AppLocalizations l) async {
    ...
    DebugLogger.log('[Playstation2GameDetailsPage] achievements reset for ${widget.gameName}');
