...
class PcGameOpen extends StatefulWidget {
  const PcGameOpen({super.key, required this.gameName});

  final String gameName;

  @override
  State<PcGameOpen> createState() => _PcGameOpenState();
}

class _PcGameOpenState extends State<PcGameOpen> {
  ...
  Future<int> _loadSavedPlaytime() async {
    final file = File(getGamePlaytimePath('PC', widget.gameName));
  ...
  Future<void> _savePlaytime() async {
    ...
    DebugLogger.log('[PcGameOpen] saved playtime: ${total}s total');
  }
  ...
  Future<void> _openGame() async {
    DebugLogger.log('[PcGameOpen] opening game: ${widget.gameName}');

    final l = AppLocalizations(LocaleService.instance.locale);
    final gameRoot = getGameRootPath('PC', widget.gameName);
    final info = await getGameInfo('PC', widget.gameName);

    if (info == null) {
      DebugLogger.log('[PcGameOpen] game_info.json not found for: ${widget.gameName}');
      if (mounted) Navigator.pop(context, l.pcGameInfoNotFound(widget.gameName));
      return;
    }

    // Resolve executable as absolute path so the launcher finds it regardless
    // of how it resolves the working directory internally.
    final resolvedCommand = [
      ...info.launcher,
      '$gameRoot/${info.executable}',
      ...info.args,
    ];

    // Merge game env vars on top of the current process environment so PATH
    ...
    try {
      final process = await Process.start(
        ...
        resolvedCommand.sublist(1),
        workingDirectory: gameRoot,
        environment: env,
      );
      DebugLogger.log('[PcGameOpen] process launched (pid: ${process.pid})');
      _gamePid = process.pid;
      GamepadService.instance.setGameRunning(true);

      // Drop loading UI after 8s — umu-run/Proton don't emit a ready signal.
      _hudFallbackTimer = Timer(const Duration(seconds: 8), () {
        if (mounted && _showLoadingUi) {
          DebugLogger.log('[PcGameOpen] fallback timer — dropping loading UI');
      ...
      process.stdout.transform(const SystemEncoding().decoder).listen(
        (line) => DebugLogger.log('[pc:stdout] $line'),
      );
      process.stderr.transform(const SystemEncoding().decoder).listen(
        (line) => DebugLogger.log('[pc:stderr] $line'),
      );
      ...
      DebugLogger.log('[PcGameOpen] process exited with code: $exitCode');

      if (!mounted) return;
      await _windowChannel.invokeMethod('forceFocus');
      if (!mounted) return;
      if (exitCode != 0 && exitCode != -15) {
        Navigator.pop(context, l.pcExitError(exitCode));
      } else {
        Navigator.pop(context);
      }
    } catch (e) {
      DebugLogger.log('[PcGameOpen] failed to launch: $e');
      _hudFallbackTimer?.cancel();
      await _savePlaytime();
      GamepadService.instance.setGameRunning(false);
      if (!mounted) return;
      await _windowChannel.invokeMethod('forceFocus');
      if (!mounted) return;
      Navigator.pop(context, l.pcLaunchError(e));
    }
  }
  ...
  Future<void> _toggleWindowFocus() async {
    ...
    DebugLogger.log('[PcGameOpen] L+R — windowFocused: $_windowFocused');
    if (_windowFocused) {
      await _windowChannel.invokeMethod('forceFocus');
      _startHudTimers();
    } else {
  ...
  List<StreamSubscription<GamepadAction>> _watchExitHold(Process process) {
    final downSub = GamepadService.instance.buttonDown.listen((action) {
      if (action == GamepadAction.start) {
        ...
        _exitHoldTimer = Timer(const Duration(seconds: 5), () {
          DebugLogger.log('[PcGameOpen] start held 5s — killing process');
          process.kill();
        });
    ...
    final upSub = GamepadService.instance.buttonUp.listen((action) {
      ...
      if (action == GamepadAction.l) _lHeld = false;
      if (action == GamepadAction.r) _rHeld = false;
    });
